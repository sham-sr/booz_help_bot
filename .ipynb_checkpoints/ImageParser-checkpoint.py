...
class YandexImage:
    ...
    def search(self, query: str, sizes: Size = 'large', sources='ya') -> list:
        ...
        if sources == 'rambler':
            request = requests.get('https://images.rambler.ru/search',
                                   params={"query": query,
                                           "image_size": sizes
                                           },
                                   headers=headers)
            ...
            try:
                ...
                soup = bs4(request.text, 'html.parser')
                output = list()
                for link in soup.find_all('div', {'class': re.compile("^Images__img")}):
                    pattern = re.compile(r'\((.+?)\)')
                    output.append(pattern.findall(link.get('style'))[0])
            except:
                return output
        return output
